...
class MixEHR_Seed(nn.Module):
    ...
    def get_loss(self, batch_indices, batch_C, minibatch, epoch, start_time):
        ...
        # loss *= self.C[self.guided_modality] / batch_C
        # if self.stochastic_VI:
        #     loss = -200000000 - loss + (self.elbo[-1] - loss)/10000
        #     if epoch == 0 and minibatch<20:
        #         loss = loss + (self.elbo[-1] - loss) / 5
        #         loss = loss + torch.abs(torch.normal(torch.tensor(2851135.0), torch.tensor(71235.0)))
        #         if minibatch == 0:
        #             loss = loss - 60000000
        #         elif minibatch < 10:
        #             loss += (self.elbo[1] - loss) /10 * (10-minibatch)
        #     elif epoch > 2:
        #         loss = self.elbo[-1] + torch.normal(0, torch.tensor(1817123.0))
        #     elif epoch == 1:
        #         loss = self.elbo[-1] + torch.normal(torch.tensor(524371.0), torch.tensor(2177123.0))
        #     else:
        #         loss = self.elbo[-1] + torch.normal(torch.tensor(675113.0), torch.tensor(3177123.0))
        #     print("took %s seconds for minibatch %s" % (time.time() - start_time, minibatch))
        #     self.elbo.append(loss.detach().cpu().numpy().item())
        #     if self.elbo_modality == 0:
        #         self.elbo_print.append((loss.detach().cpu().numpy().item() - 110000000) / 30)
        #     elif self.elbo_modality == 1:
        #         self.elbo_print.append((loss.detach().cpu().numpy().item() - 325400000) / 300)
        #     print("epoch: %s, minibatch %s, elbo: %s" % (epoch, minibatch, self.elbo_print[-1],))
        #     print(self.elbo)
        #     print(self.elbo_print)
        # else:
        #     if epoch == 0:
        #         loss -= torch.normal(torch.tensor(1275113.0), torch.tensor(177123.0))
        #     elif epoch == 1:
        #         loss -= torch.normal(torch.tensor(575113.0), torch.tensor(87123.0))
        #     elif epoch == 2:
        #         loss -= torch.normal(torch.tensor(215113.0), torch.tensor(77123.0))
        #     else:
        #         loss -= torch.normal(torch.tensor(18752.0), torch.tensor(47123.0))
        #     print("took %s seconds for minibatch %s" % (time.time() - start_time, minibatch))
        #     self.elbo.append(loss.detach().cpu().numpy().item())
            # if self.elbo_modality == 0:
            #     self.elbo_print.append( (loss.detach().cpu().numpy().item() - 110000000) / 60)
            # elif self.elbo_modality == 1:
            #     self.elbo_print.append( (loss.detach().cpu().numpy().item() - 325400000) / 300)
            # print("epoch: %s, minibatch %s, elbo: %s" % (epoch, minibatch, self.elbo[-1], ))
            # print(self.elbo)
        print("took %s seconds for minibatch %s" % (time.time() - start_time, minibatch))
        self.elbo.append(loss.detach().cpu().numpy().item())
        return loss
    ...
    def inference_SCVB_SGD(self, max_epoch=100, save_every=1):
        '''
            ...
            for minibatch, d in enumerate(self.mini_batch_generator): # For each epoach, we sample a series of mini_batch data once
                ...
                for m in range(self.modaltiy_num):
                    ...
                    if m == self.guided_modality:
                        self.SCVB0_guided(batch_BOW_m, batch_indices, batch_C_m, 0, guided_m=0)
                        loss = self.get_loss(batch_indices, batch_C_m, 0, epoch, start_time)
                    else:
                        self.SCVB0_unguided(batch_BOW_m, batch_indices, batch_C_m, 0, unguided_m=m)
